...
--- View all the movies with a parenthesis in the title 
SELECT title 
FROM MOVIES
WHERE title LIKE '%(%';


-- (Nineteen Eighty-Four) is the only title fully parenthesid
-- Fix the Movie title (Nineteen Eighty-Four)
UPDATE movies
SET title = 'Nineteen Eighty-Four'
WHERE title LIKE '%(Nineteen%';

 

--- Fix the whole movies data base to return everything but "(*)"
UPDATE movies
SET title = CASE
...
DROP TABLE IF EXISTS user2_movies;
CREATE TABLE user2_movies  AS
SELECT * 
FROM movie_and_ratings mar 
WHERE userID = 2;

SELECT * 
FROM user2_movies um
INNER JOIN binned_movie_ratings bmr
ON um.title  = bmr.title
...
------------------- Extra Credit --------------------------
---- Work on alogrithm to predict what movies the user will like 
